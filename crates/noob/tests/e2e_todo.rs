//! e2e for the agentic `todo` checklist tool through the compiled binary
//! against the mock server. Drives a model turn that emits a `todo` tool
...
fn todo_tool_renders_and_updates_a_visible_checklist() {
    ...
    rig.server.enqueue_stream_toolcalls(&[("p1", "todo", first)], None);
    rig.server.enqueue_stream_toolcalls(&[("p2", "todo", second)], None);
    ...
    assert!(r1.starts_with("plan (1/3 done):"), "header/progress wrong:\n{r1}");
    assert!(r1.contains("[x] research the codebase"), "completed glyph/item:\n{r1}");
    assert!(r1.contains("[~] write the todo tool"), "in-progress glyph/item:\n{r1}");
    ...
    assert!(r2.starts_with("plan (2/3 done):"), "progress did not advance:\n{r2}");
    assert!(r2.contains("[x] write the todo tool"), "status did not update to completed:\n{r2}");
    assert!(r2.contains("[~] add tests"), "status did not update to in_progress:\n{r2}");
