...
The active frame always shows status, editable draft or question, and queue/cancel state. Enter accepts a non-empty message as queued, leaving the running turn untouched: the message waits as a pinned `[queued]` row in the strong prompt green under the plan and agents rows, and dispatches in order once the turn ends, echoed into the transcript as a plain `› message` record at that moment (never at acceptance, so no stale `[queued]` marker survives an answer). Explicit cancellation hands queued and unsubmitted text back to the draft. Escape twice within five seconds cancels; Ctrl-C commits cancellation immediately.
