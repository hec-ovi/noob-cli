//! The unix arm of the process-runner contract: a session per child so
//! kill(-pgid) reaches the tree, WNOWAIT polling so the leader's zombie pins
//! the pgid, and a post-kill reap of reparented group members.
