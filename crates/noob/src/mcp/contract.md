...
Configuration merges `<config>/mcp.json` then `<workspace>/.noob/mcp.json` (project wins per name). `/mcp add <name> <url|command...>` writes the project file through the same entry validation the loader uses, reloads the live server set, registers the two MCP tools on the zero-to-some transition (one sanctioned cache break, like the skill tool), and appends an in-band `[mcp updated]` note. `/mcp remove` edits only the project file; `/mcp connect` is the human-facing connect that prints the catalog.
