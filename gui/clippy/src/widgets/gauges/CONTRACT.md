...
The gauge vocabulary: extent, grid, and paint, shared by the monitor panes and the context widget.

## Surface

One painter: props in, draw calls out. It reads a Vec<Gauge> from the monitor box and the layout's
...
Scene-level: this box renders its own window through `view::testkit` and reads what was drawn (4 tests in all here).
