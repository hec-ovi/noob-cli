...
The plan widget: the checklist as list rows with status glyphs.

## Surface

One painter: props in, draw calls out. It reads frame.state.plan and the layout's
...
Scene-level: this box renders its own window through `view::testkit` and reads what was drawn (0 tests in all here).
