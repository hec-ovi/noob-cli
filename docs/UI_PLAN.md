...
- Resize through SIGWINCH while idle or during an active turn resets the viewport: clear screen, home the cursor, repaint the frame. After a reflow no cursor-relative erase can reliably find the old frame. Known pending issue: repeated resizes leave stale idle frames and blank gaps in scrollback; resize handling remains under rework.
