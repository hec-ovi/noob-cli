...
- The agent installs skills itself: shown a folder or repo holding a SKILL.md, it offers to install, and each install waits on a y/n confirmation (in the terminal and in the NO0B window alike). Installed skills land in the config dir's `skills/` and are loadable in the same session.
