...
The installer seeds a global `web-search` skill. On upgrade it replaces only a copy it seeded itself, matched byte for byte against every version previously shipped, and removes only the byte-exact obsolete websearch MCP file; custom config is preserved. The skill documents the bundled Python package: `websearch init` first, then `websearch web-search`, `websearch web-fetch`, and the related subcommands, plus the opt-in `websearch tor up` layer and `--onion` searching. noob also registers a `websearch` tool whenever that CLI is on PATH, which runs the same binary with a fixed argv and no shell; `NOOB_WEBSEARCH` overrides the binary or turns the tool off. Tor and onion calls carry longer default timeouts than clearnet ones, since a bootstrap through three relays and a possible Expert Bundle download are not comparable to an HTTP request.
...
|---|---:|---:|
| Static release binary | 8 MiB | 4,465,600 bytes |
