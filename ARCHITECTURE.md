...
The installer seeds a global `web-search` skill. On upgrade it replaces only the byte-exact 0.2.6 skill noob previously seeded and removes only the byte-exact obsolete websearch MCP file; custom config is preserved. The skill documents the bundled Python package: `websearch init` first, then `websearch web-search`, `websearch web-fetch`, and the related subcommands. noob also registers a `websearch` tool whenever that CLI is on PATH, which runs the same binary with a fixed argv and no shell; `NOOB_WEBSEARCH` overrides the binary or turns the tool off.
