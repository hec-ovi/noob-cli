...
| 5 | UI | Cooked-mode plain REPL; no ratatui, no rustyline | Terminal provides native line editing; TUIs add a redraw bug class; history is a v0.2 termios micro-editor |
...
└── crates/
    ...
    │   │   └── ui/          # contract.md: cooked-mode REPL, streaming stdout writer, ANSI, y/N prompts, JSONL event emitter
...
Loaded-skill names are tracked and re-listed (names only) after compaction. The agent never authors skills in v0.1; as defense in depth, any write/edit whose path matches `**/skills/**` requires confirmation in every mode including auto (agent-created skills are persistent injection vectors; in headless/child contexts where there is no TTY, this ask degrades to deny).
...
**Cut from v0.1** (each with its upgrade path recorded here): CRC hashedit (check-and-set covers the safety property; hashedit slots behind the same tool name), Anthropic API adapter (the neutral Turn/Event types and per-shape adapters accommodate it), ratatui or any TUI, line editor and history (v0.2 termios micro-editor), markdown rendering, permission glob-rule DSL (the container is the wall), first-run wizard (autodetect + .env.example instead), agent-authored skills (deliberately never without an explicit user gate), MCP resources and prompts, `resend_reasoning` and any persisted compat registry (process-lifetime cache only), background bash, LSP anything, token-level child budgets (turn + wall-clock caps instead), network-isolated compose profile (documented as future work), secret scanner, aarch64 image (P7 stretch), Windows.
