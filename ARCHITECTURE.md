...
Releases carry one static binary per Linux architecture (x86_64 and aarch64, musl), packaged as a deb and a tarball; the release workflow builds and publishes both on tag, and it is the one CI surface (tests run locally). Installed natively, noob runs in workspace mode: the file tools refuse paths outside the project, every command the model types is folder-locked by the kernel (Landlock, see the exec contract), and configuration and sessions live under `~/.config/noob`. `noob --resume <id>` (aliases `--restore` and `--session`) resumes a saved session.

Web search is the `websearch-skill` package: `uv tool install websearch-skill` provides the CLI, and the skill installs from `hec-ovi/websearch-skill`. noob registers a `websearch` tool whenever that CLI is on PATH, which runs the binary with a fixed argv and no shell; `NOOB_WEBSEARCH` overrides the binary or turns the tool off. noob pins the tool's dotenv at the config directory (`WEBSEARCH_ENV_FILE`), so a project `.env` never feeds a process that opens sockets. Tor and onion calls carry longer default timeouts than clearnet ones, since a bootstrap through three relays and a possible Expert Bundle download are not comparable to an HTTP request.
...
Sandbox selection is explicit `NOOB_SANDBOX` first, then container detection. Container mode permits workspace tools to use the mounted environment. Workspace mode confines write and edit targets to the project, rejects symlink escapes, and folder-locks every bash command with Landlock where the kernel offers it, redirecting toolchain caches under temp so builds keep working. `--yolo` explicitly lifts all of that.
...
1. `# Agent`: `AGENTS.md` from the config directory, fenced in `<instructions>`. Absent, the shipped prompt is used. It carries identity, working style, tool usage, and when to reach for the plan tool and for sub-agents; a present file replaces all of it, because noob has no paired model and that reinforcement is model-specific.
2. `<env>`: environment facts.
3. `# Project instructions`: the working directory's `AGENTS.md`, fenced the same way.
4. `# Skills`: the resolver line and the SKILL.md index, in `<available_skills>`, when skills exist.
5. `# MCP servers`: the configured names, in `<mcp_servers>`, when any exist.

Markdown headings name every layer; tags fence every text this process did not write, so a path or a project's instructions cannot read as part of ours. The user's file is capped at 16 KiB. `noob doctor` says which prompt text is in effect.
...
- A tool round consisting solely of successful detached spawn acknowledgments ends the input. The parent has nothing to wait for (reports restart inference on their own), and a small model left in-turn after spawning reliably invents a wait idiom - status polls, bash `ls` probes of the child's expected output, leading sleeps - that spams the scrollback and delays the human's queued follow-up. Mixed batches (a spawn plus real work, controls, or snapshots) keep the turn, and a ready report integrated with the batch also keeps it. The acknowledgment contract tells the model to spawn every sibling agent in one response.
- Status polling is capped structurally. The first tool round in an input consisting only of `subagent {"status":true}` calls is answered with a real snapshot (whose digest carries the longest-running child's elapsed), so a "how are the agents doing?" question stays answerable. From the second status-only round on, the calls receive a calm canned result and the input ends after the batch, freeing the prompt; if a ready report arrived with that batch, the input continues so the model can process it. Detached subagent digests carry no per-call duration (a hub control returns in microseconds and a constant "0.0s" read as a broken child elapsed).
...
The core tools are `read`, `write`, `edit`, `bash`, `grep`, `glob`, `ls`, `context`, and `plan`. Conditional tools are `websearch`, `skill`, `mcp_connect`, `mcp_call`, and `subagent`.
...
- `bash` and `websearch` share the same process supervisor: merged stdout and stderr, continuous bounded output collection, and process-group cleanup on timeout or cancellation. `websearch` receives a fixed argv built from typed fields and never invokes a shell.
...
- `NOOB_TOOL_CAPS=0` (or `off`) lifts every truncation cap for the session: read pages, bash output, grep matches, glob/ls entries, skill bodies, websearch output, and MCP results flow through whole and no truncation marker renders. Resolved once at bootstrap into the shared tool context; children read their own configuration, so the setting propagates through the environment.
...
Child tool profiles are structural. `tools: "read-only"` exposes `read`, `grep`, `glob`, `ls`, `context`, and `skill` when a skill is available. `tools: "web"` adds the `websearch` tool and requires the CLI behind it; a child in this mode gets no MCP servers at all, since the configured set is the user's and a restricted researcher must not inherit it. It exposes no Bash, write, edit, plan, or subagent tool: the web capability is one countable tool rather than a shell, which is also what the evidence gate counts. `tools: "all"` exposes the full registered set; a dock child is still a leaf. Loaded ancestor skills are excluded from child discovery so orchestration instructions cannot recursively activate themselves.

A completed `tools: "web"` report is accepted only after at least two successful source-gathering `websearch` calls, counted live as results arrive so child-side compaction cannot erase the evidence. Validation failures, nonzero exits, `init`, and `doctor` do not count. If the first completion has fewer, noob gives that same child one corrective instruction to gather usable search and source evidence, using only the unused part of the original inference-round budget. An exhausted budget or a second completion still below the evidence threshold returns a child error. Aborted and interrupted runs are not retried.
...
Defaults are four concurrent children, 25 rounds, no wall clock (set `NOOB_TASK_WALL_CLOCK_S` to a positive number of seconds to cap a child), and recursion depth two on inline surfaces. Dock children are leaves. The parent starts the wall clock before sending the task. Child stdin is nonblocking and cancellation-aware. The live dock retains the newest 2 KiB and at most 12 stderr lines per child; `--verbose` retains at most 64 KiB as completion diagnostics. Final stdout results are uncapped, and both pipes always drain.
...
Entering a message during a turn queues it as a pinned row inside the frame, styled like the record it will become with a green `[queued]` tag, and touches nothing: the parent turn, its tools, the plan, and every detached child keep running, and queued messages dispatch in order on the next REPL loops once the turn ends on its own, each echoed into the transcript as a plain record at dispatch (the `[queued]` marker disappears with the pinned row, so it can never outlive its answer). Only an explicit Escape-Escape or Ctrl-C stops a turn, and cancellation hands queued and unsubmitted text back to the draft. Escape twice within five seconds cancels; Ctrl-C cancels immediately; a second Ctrl-C during cancellation restores the terminal and exits with status 130.

Double-Escape is the stop-everything gesture: committing it also cancels every detached child (each still delivers its one terminal packet), and the interrupt note says "stopping N detached agents" instead of claiming they keep running. At the idle prompt with agents active, a first Escape arms a visible "press ESC again to stop all agents" hint and a second Escape inside the five-second window stops the fleet without touching the draft. A lone Ctrl-C keeps its narrower contract: it stops only the parent turn and the fleet keeps running.

Reader loss is an ordering barrier. Keys accepted before EOF are processed first, pending confirmations are denied, and any already queued message is dispatched before the REPL exits. It never waits on an input thread that can no longer answer.
...
| Fixed prompt plus schemas | 1,980 tokens | 1,969 o200k with all 14 tools registered |
